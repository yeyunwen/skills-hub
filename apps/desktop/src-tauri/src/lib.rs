...
use skills_hub_core::{
    ...
    discover_ssh_hosts as core_discover_ssh_hosts, get_preferences as core_get_preferences,
    get_skill_detail as core_get_skill_detail, init_hub as core_init_hub,
    ...
    remote_import_skill as core_remote_import_skill, remote_list as core_remote_list,
    remote_remove_skill as core_remote_remove_skill, remote_scan as core_remote_scan,
    remote_sync_local_agent_skill as core_remote_sync_local_agent_skill,
    remote_sync_skill as core_remote_sync_skill, remote_sync_plan,
    remove_agent_skill as core_remove_agent_skill, remove_hub_skill as core_remove_hub_skill,
    remove_remote as core_remove_remote,
    ...
    takeover_agent_skill as core_takeover_agent_skill, unlink_skill as core_unlink_skill,
    update_preferences as core_update_preferences, AgentKind, InstallOptions, SyncMethod,
};
use std::{
    fs::{self, File},
    io::Read,
...
async fn remote_sync_skill(input: RemoteSkillInput) -> Result<serde_json::Value, String> {
    run_blocking(move || {
        let agent = parse_agent(&input.agent)?;
        let method = parse_sync_method(input.sync_method.as_deref().unwrap_or("auto"))?;
        let message = format!("同步远程单个 Skill：{} {} {}", input.name, input.agent, input.skill_name);
...
#[tauri::command]
async fn remote_sync_local_agent_skill(input: RemoteLocalAgentSkillInput) -> Result<serde_json::Value, String> {
    run_blocking(move || {
...
async fn remote_import_skill(input: RemoteSkillInput) -> Result<serde_json::Value, String> {
    run_blocking(move || {
        let agent = parse_agent(&input.agent)?;
        let message = format!("导入远程 Skill：{} {} {}", input.name, input.agent, input.skill_name);
...
async fn remote_remove_skill(input: RemoteSkillInput) -> Result<serde_json::Value, String> {
    run_blocking(move || {
        let agent = parse_agent(&input.agent)?;
        let message = format!("移除远程 Skill：{} {} {}", input.name, input.agent, input.skill_name);
